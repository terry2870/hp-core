 ...
 */
(function($) {
	$.fn.jqForm = function(options, param) {
		var self = this;
		if (typeof (options) == "string") {
			var method = $.fn.jqForm.methods[options];
			if (method){
				return method.call(this, param);
			}
		}
		return this.each(function() {
			var opt = $.extend({}, $.fn.jqForm.defaults, options);
			self.data("jqForm", opt);
			_createForm(self);
		});
	};
	...
	function _submit(jq, submitParam) {
		var opt = $.extend({}, jq.data("jqForm"), submitParam);
	...
	$.fn.jqForm.defaults = $.extend({}, $.fn.jqForm.event, {
		loadDepend : [],		//加载前依赖于
		id : "",				//id
		name : "",				//name
		submitUrl : "",			//提交的url
